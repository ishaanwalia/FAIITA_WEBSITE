            ...
            <ScrollReveal key={r.id} direction="up" delay={i * 0.06}>
              <TiltCard maxTilt={5}>
                <GlassCard variant="light" className="flex h-full flex-col">
                  ...
                  <h3 className="mt-3 font-display text-lg font-bold text-navy-800">{r.title}</h3>
                  <p className="mt-2 flex-1 text-sm leading-relaxed text-muted-foreground">{r.description}</p>
                  <a
                    href={r.fileUrl ?? "#"}
                    className="mt-5 inline-flex w-fit items-center gap-2 rounded-full border border-navy-700/20 px-4 py-2 text-sm font-semibold text-navy-700 hover:bg-navy-700/5"
                  >
